...
certificateRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      const body = await convertToObject(
        req.params.id,
        req.body,
        req.files,
        url
      )
      const newcertificate = await certificateController.addcertificate(
        req.params.id,
        body
      )
...
certificateRouter.get('/getcertificatedetails/:id/:type', async (req, res) => {
  try {
    ...
    const allCertificates = await certificateController.getcertificateByEventId(
      id,
      type
    )
    return res.status(200).json(allCertificates)
...
certificateRouter.get('/:certificateId', async (req, res) => {
  try {
    ...
    const certificate = await certificateController.getcertificateById(
      certificateId
    )
...
certificateRouter.put(
  ...
  async (req, res) => {
    try {
      ...
      const updatedone = await certificateController.updatecertificate(
        certificateId,
        updatedCertificate
      )
...
certificateRouter.delete(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ response: 'Certificate deleted successfully' })
    } catch (e) {
      return res
        .status(e?.status || 500)
        .json({ error: e?.message || 'Internal Server Error' })
...
certificateRouter.delete(
  ...
  async (req, res) => {
    try {
      ...
      const response = await userEventService.deleteSignature(
        userId,
        signatureUrl
      )
